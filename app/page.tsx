            ...
            if (loadFromUrl) {
                ...
                try {
                    ...
                    if (Array.isArray(parsedPhotos) && (parsedPhotos.length === 0 || parsedPhotos.every(p => p.id && p.imageUrl))) {
                        ...
                        throw new Error("Invalid photo data in shared link.");
                    }
                } catch (error) {
    ...
    const handleFileSelect = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (jsonFiles.length > 0) {
            ...
            try {
                ...
                if (photosToLoad.every(p => p.id && (p.imageUrl || p.dataUrl))) {
        ...
        if (!loadedTimeline && imageFiles.length > 0) {
            for (const file of imageFiles) {
                const tempId = `temp-${++idCounterRef.current}`;
                try {
    ...
    const randomPositions = useMemo(() => {
        if (!isMounted) {
            return Array(photos.length + 1).fill(0).map((_, index) => ({
                x: (index - photos.length / 2) * 120,
                y: 0,
                rotate: 0,
            }));
        }
        
        // Use deterministic "random" based on index for hydration consistency
        ...
        return Array(photos.length + 1).fill(0).map((_, index) => ({
            x: (index - photos.length / 2) * 120 + (simpleRandom(index * 7) - 0.5) * 150,
                            ...
                            >
                                <DraggableCardContainer>
                                    <DraggableCardBody dragConstraintsRef={dragAreaRef} className="!p-0 !bg-transparent !shadow-none">
                                        <PolaroidCard
                                            {...photo}
                                            dataUrl={isSharedView ? (photo.generatedUrl || photo.imageUrl || photo.localDataUrl || '') : (photo.localDataUrl || photo.imageUrl || '')}
