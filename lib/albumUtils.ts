...
function loadImage(src: string): Promise<HTMLImageElement> {
    return new Promise((resolve, reject) => {
        const img = new Image();
        // Setting crossOrigin is crucial for loading images from cloud storage onto a canvas
...
export async function createAlbumPage(imageData: Record<string, string>): Promise<string> {
    ...
    ctx.fillText('Generated with First Anniversary', canvasWidth / 2, 150);
 ...
 * This helps reduce the storage size for localStorage.
 * @param file The image file to resize.
 * @param maxSize The maximum width or height of the resized image.
 * @returns A promise that resolves to a data URL of the resized image (JPEG format).
 */
export function resizeImage(file: File, maxSize: number = 1024): Promise<string> {
    return new Promise((resolve, reject) => {
